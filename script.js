async function loadText(url){
  ...
  contactName.textContent = `Ansprechperson: ${config.contact.name || '–'}`;
  contactEmail.textContent = `E-Mail: ${config.contact.email || '–'}`;

  const hasContact = !!(config.contact.name || config.contact.email);
  contactCard.style.display = hasContact ? 'block' : 'none';

  dataPopupTitle.textContent = `Datenübersicht: Leipzig (Stadt) – ${config.label}`;
  dataPopupHint.textContent = 'Übersicht der Datenarten und des Pflegesystems für diese Einheit.';
  dataPopupList.innerHTML = config.rows.map(r => `
    <div class="data-row">
      <div class="data-type">${escapeHtml(r.type || '–')}</div>
...
modeConventionBtn.addEventListener('click', () => {
  if(activeId === 'leipzig_stadt'){
    setLeipzigMode('convention');
  }
});

modeOnlineBtn.addEventListener('click', () => {
  if(activeId === 'leipzig_stadt'){
    setLeipzigMode('online');
  }
});
...
const LEIPZIG_STADT_MODES = {
  convention: {
    ...
    contact: {
      ...
      email: 'convention@example.de'
    },
    rows: [
      { type: 'Stammdaten (City POIs)', system: 'DMO-CRM / Datenbank' },
      { type: 'Veranstaltungen (City)', system: 'Event-Management-System' },
      { type: 'Medien (City)', system: 'DAM' }
    ]
  },
  online: {
    ...
    contact: {
      name: 'Name Online-Abteilung',
      email: 'online@example.de'
    },
    rows: [
      { type: 'Website', system: 'CMS' },
      { type: 'Content', system: 'SaTourN' },
      { type: 'Medien', system: 'DAM' }
    ]
  }
};
...
if(id === 'leipzig_stadt'){
  ...
  setLeipzigMode(activeLeipzigMode);
  return;
}else{
